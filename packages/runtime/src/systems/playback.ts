...
import {
	...
	AudioDecoderHandle, AudioBusHandle, VideoDecoderHandle, Host,
...
function advancePlayhead(world: World, entity: Entity): void {
	...
	let looped = false;
	const previousTime = computed.localTimeInSeconds[eid] ?? 0;
	...
	if (playback.playing[eid] === true) {
		...
		} else if (speed < 0 && time <= minSeconds) {
			if (playback.loop[eid] === true) {
				time = maxSeconds;
				looped = true;
			} else {
	...
	// Re-anchor audio after seeks and rate changes as well as play/stop.
	...
	const playing = playback.playing[eid] === true && speed > 0;
	const wasPlaying = audioPlayback.wasPlaying[eid] ?? false;
	const discontinuity = speed !== audioPlayback.speed[eid]
		|| previousTime !== audioPlayback.lastTimeInSeconds[eid];

	if (wasPlaying && (!playing || discontinuity || looped)) {
		resetDecoders(world, entity);
	}
	if (playing && (!wasPlaying || discontinuity || looped)) {
	...
	audioPlayback.speed[eid] = speed;
	audioPlayback.lastTimeInSeconds[eid] = computed.localTimeInSeconds[eid] ?? 0;
}
...
function forwardAudioDecoder(world: World, scene: Entity, entity: Entity, audioSource?: Entity): void {
	const audioEntity = audioSource ?? entity;
	const state = scene.get(Playback);
	if (entity.has(Muted) || entity.get(Computed)?.visibility !== 1
		|| !state?.playing || state.speed <= 0) {
		audioEntity.get(AudioDecoderHandle)?.reset();
		return;
	}

	const resolvedDecoder = resolveAudioDecoder(world, audioEntity);
	...
	const speed = playback.speed[sid] || 1;
	const playbackRate = (computed.playbackRate[eid] || 1) * speed;
	...
	const audioDelay = audioOffset - playbackOffset / speed;
	...
	if (!decoder.ready) {
		...
		const playPromise = decoder.playTo(bus, {
			...
			relativeTo: localFrame / fps + 0.5 * playbackRate,
			trimStart: source.in / fps,
			trimEnd: source.out / fps,
			playbackRate,
			currentTime: currentTime / fps,
			relativeDelay: (origin / fps) / speed + audioDelay,
		});
