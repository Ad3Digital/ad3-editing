...
/** K leaves shuttle mode playing forward at 1x; otherwise it toggles play/pause. */
export function toggleShuttlePlayback(world: World, scene: Entity): void {
	const playback = scene.get(Playback);
	if (!playback) return;

	if (playback.playing && playback.speed !== 1) {
...
/** Each J/L press advances one shuttle tier, never changing the authored clip rate. */
export function shuttlePlayback(world: World, scene: Entity, direction: -1 | 1): void {
	const playback = scene.get(Playback);
	if (!playback) return;

	const speed = playback.playing && Math.sign(playback.speed) === direction
		? Math.min(5, Math.max(2, Math.abs(playback.speed) + 1))
		: 2;
	const computed = scene.get(Computed);
	const workarea = scene.get(Workarea);
	const start = workarea?.start ?? 0;
	const end = workarea?.end ?? computed?.duration ?? 0;
	const frame = computed?.localTime ?? 0;

	if (direction > 0 && (frame < start || frame >= end)) {
		setPlayhead(world, scene, start);
	} else if (direction < 0 && (frame <= start || frame >= end)) {
		setPlayhead(world, scene, Math.max(start, end - 1));
	}

	scene.set(Playback, { playing: end > start, speed: direction * speed });
	const context = world.get(AudioEngine)?.context;
	if (context instanceof AudioContext) void context.resume();
}
