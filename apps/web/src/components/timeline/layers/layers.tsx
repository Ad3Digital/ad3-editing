...
export function Layers() {
  ...
  const playbackStatus = createMemo(() => {
    const speed = playback()?.speed ?? 1;
    return `${speed < 0 ? '◀' : '▶'} ${Math.abs(speed)}×`;
  });
      ...
      >
        ...
        >
          ...
          <Tooltip placement="top">
            <TooltipTrigger<'span'>
              as="span"
              class="min-w-8 text-center text-xxs font-mono text-muted-foreground tabular-nums"
              aria-label={`Playback ${playbackStatus()}`}
            >
              {playbackStatus()}
            </TooltipTrigger>
            <TooltipPortal>
              <TooltipContent>J / L shuttle (2×–5×). K restores forward 1×, then play/pause.</TooltipContent>
            </TooltipPortal>
          </Tooltip>
          <Tooltip placement="top">
            <TooltipTrigger<typeof Button>
