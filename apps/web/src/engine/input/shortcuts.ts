...
/** Starts or accelerates a directional shuttle for the active scene. */
function shuttleActivePlayback(world: World, direction: -1 | 1): void {
	const scene = getActiveEntity(world);
	if (scene) shuttlePlayback(world, scene, direction);
}

/** Restores forward 1× playback from a shuttle, then toggles normal playback. */
function toggleActiveShuttlePlayback(world: World): void {
	const scene = getActiveEntity(world);
	if (scene) toggleShuttlePlayback(world, scene);
}
...
const PRESSED_SHORTCUTS: readonly Shortcut[] = [
	...
	{ keys: ['j', '!mod', '!shift', '!alt'], action: (world) => shuttleActivePlayback(world, -1) },
	{ keys: ['k', '!mod', '!shift', '!alt'], action: toggleActiveShuttlePlayback },
	{ keys: ['l', '!mod', '!shift', '!alt'], action: (world) => shuttleActivePlayback(world, 1) },
