...
const EXPORT_RENDER = { context: { timeoutMs: 60 * 60 * 1000 } };

const APP_NAME = "AD3 Editing";
...
function launchApp(background: boolean): Promise<boolean> {
  if (process.platform === "darwin") {
    ...
    execFile("open", args, (error) => resolve(!error));
    return promise;
  }

  if (process.platform === "win32") {
    const appRoot = process.env.DIFFUSION_APP_PATH;
    if (!appRoot) return Promise.resolve(false);
    const executable = join(appRoot, `${APP_NAME}.exe`);
    if (!existsSync(executable)) return Promise.resolve(false);

    const env = { ...process.env };
    delete env.ELECTRON_RUN_AS_NODE;
    const { promise, resolve } = Promise.withResolvers<boolean>();
    const child = spawn(executable, background ? ["--hidden"] : [], {
      detached: true,
      env,
      stdio: "ignore",
...
async function openProject(path: string | undefined, opts: OpenOptions): Promise<void> {
  // The packaged wrappers can launch their respective desktop application.
  // A development CLI has no reliable application path, so it falls through
  // to the socket and reports that the editor must be started first.
...
type ExportOptions = { resolution?: string; fps?: string };

async function exportVideo(output: string, opts: ExportOptions): Promise<void> {
  const resolution = opts.resolution === undefined ? undefined : Number(opts.resolution);
  if (resolution !== undefined && (!Number.isInteger(resolution) || resolution <= 0)) {
    console.error(`--resolution must be a positive integer (got "${opts.resolution}")`);
    process.exit(1);
  }

  const fps = opts.fps === undefined ? undefined : Number(opts.fps);
  if (fps !== undefined && (!Number.isFinite(fps) || fps <= 0)) {
    console.error(`--fps must be a positive number (got "${opts.fps}")`);
    process.exit(1);
  }

  try {
    const result = await editor.export.mutate(
      { output: resolve(output), resolution, fps },
      EXPORT_RENDER,
    );
    console.log(JSON.stringify(result));
  } catch (e) {
...
program
  .name("dapi")
  .description(
    `The AD3 Editing CLI: understand, generate, and edit footage.
...
program
  .command("export")
  .description(
    `Render the active scene directly to an MP4 path without a save dialog. Uses H.264/AAC and streams through the desktop app, preserving the project timeline, shaders, audio and export progress.`,
  )
  .argument("<output>", "output .mp4 path")
  .option("-r, --resolution <pixels>", "output height in pixels (default: 1080)")
  .option("--fps <fps>", "output frame rate (default: project frame rate)")
  .action((output: string, opts: ExportOptions) => exportVideo(output, opts));
